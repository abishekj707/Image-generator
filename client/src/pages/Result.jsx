...
const Result = () => {
  ...
  const onSubmitHandler = async (e) => {
    e.preventDefault() //it will prevent the page from reloading when we submit the form
    setLoading(true)

    if(input){//if the input is available
      const image = await generateImage(input) //we will call the generateImage function from appContext and pass the input as argument to generate the image, and we will get the response as image
      setIsImageLoaded(true) //after generating the image successfully we will set isImageLoaded to true to show the download and generate another button
      setImage(image) //we will set the image state variable with the generated image to show the generated image in the UI
    }
    setLoading(false) //after generating the image we will set loading to false to hide the loading text
  }
